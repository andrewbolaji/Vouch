...
check_count() {
  ...
  (( actual < expected )) \
    || return 0
  block_static "$suite reports $actual passing tests, down from a baseline of $expected. Tests were removed or skipped. Restore them, or raise the baseline in .codex/hooks/run-tests.sh with a reason."
}

# 1. Flutter suite, no emulator needed.
if ! flutter test --exclude-tags=golden >"$LOG_DIR/flutter.log" 2>&1; then
  block "flutter test --exclude-tags=golden" "$LOG_DIR/flutter.log"
fi
check_count "flutter test --exclude-tags=golden" \
