...
// The "Waitlist signup logic" suite that used to sit here has been
// deleted rather than moved, and that is the honest word for it.
// Every one of its five tests built its own document id, wrote its
// own document and then asserted that Firestore had stored what the
// test wrote. The honeypot case simulated the handler with an
// `if (!website)` inside the test body. None of them called
// waitlistSignup, so all five passed against an endpoint with no
// size caps and no rate limit, and all five still pass with the
// handler's validation removed entirely. Measured, not asserted:
// see the commit that added finding 15's caps.
//
// The coverage they claimed now lives in waitlist.test.ts, which
// calls waitlistSignup(req, res) itself.
