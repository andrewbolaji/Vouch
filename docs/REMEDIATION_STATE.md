...
| 5 | **Built in three commits, one deployed.** `10cc6ce` event id dedupe plus the ordering guard that stops a retried EXPIRATION downgrading a resubscribed user, **deployed to production 2026-08-17 and verified ACTIVE**. `a594b6d` the `GET /subscribers` reconciliation and the client call behind the pending screen's retry button, **not deployed**: it declares `REVENUECAT_REST_API_KEY`, which does not exist in Secret Manager, and a function declaring a missing secret fails at deploy. `2679757` signature verification, built and held, inert until `REVENUECAT_WEBHOOK_SIGNING_SECRET` exists, and `REVENUECAT_SIGNATURE_HEADER` must be confirmed against RevenueCat's documentation before that secret is set. Secret Manager currently holds exactly one secret, `REVENUECAT_WEBHOOK_SECRET`. | `10cc6ce`, `a594b6d`, `2679757` |
| 6 | **Design written, not built:** `docs/FINDING_6_DESIGN.md`, 2026-08-17. Job document as the mechanism, scheduled resume over stale jobs as the backstop, four steps each idempotent. The thing the design turned up: Andrew's ordering puts auth deletion last, which means `onUserDeleted` can no longer be the driver and the entry point becomes a callable, which in turn means the `requires-recent-login` protection has to be re-implemented as an `auth_time` check or account deletion becomes available to anyone holding a stolen unexpired token. Session revocation is also weaker than it sounds, since an issued ID token stays valid up to an hour and rules cannot see revocation, so the design adds a second cascade pass after the auth record is gone rather than paying a per-write rules read forever. Three questions open for Andrew at the end of the doc. | |
