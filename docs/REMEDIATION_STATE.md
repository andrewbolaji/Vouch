...
| 5 | **Built locally, one production portion remains active, two activation gates remain manual.** `10cc6ce` event id dedupe plus the ordering guard that stops a retried EXPIRATION downgrading a resubscribed user was deployed on 2026-08-17 and verified active. The `GET /subscribers` reconciliation and client repair path remain switched off by `RECONCILE_ENABLED` until `REVENUECAT_REST_API_KEY` exists. Signature verification was corrected on 2026-08-18 to RevenueCat's documented `X-RevenueCat-Webhook-Signature` timestamped format, exact raw bytes and five-minute tolerance, with 17 focused tests, but remains switched off by `SIGNATURE_ENABLED` until the one-time signing secret exists and a RevenueCat test delivery proves the deployed path. Both switches remain plain constants because a declared missing secret blocks source analysis for every Function. Client refresh and restore now preserve the last confirmed tier when RevenueCat cannot answer instead of treating failure as an empty subscription set. | `10cc6ce`, `a594b6d`, `2679757` plus the 2026-08-18 v1 correction |
