...
```bash
flutter pub get
flutter test --exclude-tags=golden   # 402 tests, ~30s, no emulator needed
```
...
firebase emulators:exec --only firestore,auth --project vouch-test \
  'cd functions && npx jest --forceExit'        # 222 tests

(cd test-rules && npm run test:emulator)       # 112 tests
```
...
- Read the relevant active entries in `docs/LEARNINGS.md` before repeating an investigation or changing a previously fragile path.
- For material UI work, read the current contract and decisions in `docs/INTERFACE.md` before changing the interface.
- For every normal or high-risk change, and for other multi-step state-changing work, start and maintain a timestamped entry in `docs/EXECUTION.md`. Record actions, commands, targets, outcomes, artifacts, blockers and final state. Never record chain-of-thought, secrets, personal data, raw production payloads or sensitive command output.
- Add a learning only when a reproduced failure, measurement, incident or verified correction produces a reusable rule. Promote mature lessons into tests, code, repository guidance, decisions or runbooks.
- Update interface memory when a material product or design decision changes. Include applicable loading, empty, partial, offline, error, success, disabled and permission-denied states, plus accessibility and screenshot evidence.
- Run `./scripts/check-project-memory.sh` after changing the memory contract. Use `./scripts/check-project-memory.sh --staged` before committing staged implementation changes. A confirmed documentation-only or mechanical change may use `PROJECT_MEMORY_LANE=0`.
...
- Posting a comment goes through the `submitComment` callable, not a direct Firestore write, so a cold function instance can take a few seconds to respond. `restaurant_detail_screen.dart` guards this with `_isSubmittingComment`: the send button disables and shows a spinner while a submission is in flight, so a slow cold start reads as "working" rather than inviting a second tap that fires a duplicate comment.
...
4. Any change under `functions/` passes whole-codebase `firebase deploy --dry-run` source analysis. A targeted function deploy is not sufficient because every declared secret is validated before target filtering.
5. Diff self-reviewed before offering it.
6. UI changes verified by screenshot.
7. Committed **and pushed**. Uncommitted work does not exist.
